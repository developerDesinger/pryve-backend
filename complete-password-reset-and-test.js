...
const USER_CREDENTIALS = {
    ...
    password: '12345678a',
    otp: '306723' // From the previous response
};
...
async function verifyOTPAndSetPassword() {
  ...
  try {
    // First, let's try to verify the OTP
    const verifyUrl = `${BASE_URL}/api/v1/users/verify-reset-otp`;
    const verifyResult = await makeRequest(verifyUrl, 'POST', {
      email: USER_CREDENTIALS.email,
      otp: USER_CREDENTIALS.otp,
      password: USER_CREDENTIALS.password
    });
    
    console.log(`📊 Verify OTP Status: ${verifyResult.status}`);
...
async function main() {
  console.log('🚀 Complete Password Reset and Test Endpoint');
  ...
  const verifyResult = await verifyOTPAndSetPassword();
  ...
  if (verifyResult && verifyResult.success) {
    ...
    console.log('💡 The OTP might be expired or incorrect');
  }
